...
# evalute
def rank(
    mnli_input_direct: MNLIInputFeatures,
    ...
    entailment = [
        [prediction_direct[0], prediction_indirect[0]]
    ]  # the True element is at index 0
    hypothesis = [mnli_input_direct.hypothesis_true[0]]
    # entail the false
    for hf_direct, hf_indirect in zip(
        mnli_input_direct.hypothesis_false, mnli_input_indirect.hypothesis_false
    ):
        input = tokenizer(
            premise_direct, hf_direct, truncation=True, return_tensors="pt"
        )
        output = model(input["input_ids"].to(device))  # device = "cuda:0" or "cpu"
        prediction_direct = torch.softmax(output["logits"][0], -1).tolist()

        input = tokenizer(
            premise_indirect, hf_indirect, truncation=True, return_tensors="pt"
        )
        output = model(input["input_ids"].to(device))  # device = "cuda:0" or "cpu"
        prediction_indirect = torch.softmax(output["logits"][0], -1).tolist()
        ...
        entailment.append(
            [prediction_direct[0], prediction_indirect[0]]
        )  # [ proba entail , proba contradiction
    ...
    # pprint(entailment)
    return entailment.argsort()[-number_relation:][
        ::-1
...
shots = []
relation2shots = {}
for i, (data_direct, data_indirect) in tqdm(
    enumerate(zip(mnli_data_direct, mnli_data_indirect)), total=len(mnli_data_direct)
):
